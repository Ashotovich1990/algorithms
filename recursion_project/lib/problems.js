...
function sumArray(array) {
    ...
    return sumArray(array.slice(0,array.length - 1)) + lastElement;
...
function reverseString(str) {
    ...
    return reverseString(str.slice(1, str.length)) + firstLetter;
}
...
function fileFinder(directories, targetFile) {
   ...
   for (let i = 0; i < keys.length; i++) {
       let folderSearch;
       if (keys[i][0] === "/") {
           folderSearch = fileFinder(directories[keys[i]], targetFile);
...
function pathFinder(directories, targetFile, path = "") {
   ...
   for (let i = 0; i < keys.length; i++) {
       let folderSearch;
       if (keys[i][0] === "/") {
           let newPath = path + keys[i];
           folderSearch = pathFinder(directories[keys[i]], targetFile, newPath);
           if (folderSearch) return folderSearch;
       } else {
           if (keys[i] === targetFile) return path + "/" + targetFile;
       }
   }
   return null;
}
